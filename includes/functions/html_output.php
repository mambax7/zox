...
// the image filename as default
  	if(isset($_GET['zox_block'])){
    $image = '<img src="' . DIR_WS_CATALOG . zen_output_string($src) . '" alt="' . zen_output_string($alt) . '"';
  	}else{
 ...
 */
  function zen_image($src, $alt = '', $width = '', $height = '', $parameters = '') {
    global $template_dir, $request_type;
...
// the image filename as default
  	if(isset($_GET['zox_block'])){
  	  if(ENABLE_SSL == 'true' && HTTPS_SERVER != '' && $request_type == 'SSL'){
        $image = '<img src="' . HTTPS_SERVER . DIR_WS_CATALOG . zen_output_string($src) . '" alt="' . zen_output_string($alt) . '"';
      }else{
